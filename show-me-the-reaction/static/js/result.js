...
var chart3 = AmCharts.makeChart("chartdiv3", chart_data3);

firebase.initializeApp(config);
var database = firebase.database();
var usr_anger, usr_joy, usr_surprise, usr_sorrow, usr_none;
var firebase_anger, firebase_joy, firebase_surprise, firebase_sorrow, firebase_none, firebase_success, firebase_fail, firebase_cnt;
...
$(function() {
    ...
    $('#result_btn').click(function() {
        ...
        if(requestNum == 0) {
            ...
            console.log('CHALLENGE 결과!');

            element.innerHTML = (challenge_result == true) ? "[내 결과]<br/>도전 성공!" : "[내 결과]<br/>도전 실패!";
            ...
            //doFirebaseService(database.ref('/challenge/joy/video1'));
            doFirebaseService(database.ref('/challenge/'+category+'/'+video));
    ...
    function getCurVideoInfo(category, videoName) {
        ...
        database.ref().child('challenge/'+category+'/'+videoName).on('child_added', function(dataSnapshot) {
            ...
            switch (firebase_child_idx) {
                ...
                case 6:
                    firebase_success = message;break;
                case 7:
                    firebase_surprise = message;
                    ...
                    chart_data3["dataProvider"][0]["value"] = firebase_success
                    chart_data3["dataProvider"][1]["value"] = firebase_fail
                    chart3 = AmCharts.makeChart("chartdiv3", chart_data3);
                    ...
                    console.log('FSuccess : ' + firebase_success + 'FFail : ' + firebase_fail);
                    ...
                    if(is_check_result !== true) // 파이어베이스에 한번만 업로드하기
                        updateResult( database.ref('/challenge/'+category+'/'+videoName) );
	...
	function updateResult(postRef) {
	  postRef.transaction( function(post) {
		if (post) {
		      ...
		      post.none += usr_none;
		      post.success_num += (challenge_result == true) ? 1 : 0;
		      post.fail_num += (challenge_result == true) ? 0 : 1;
		      post.cnt += cnt;
		} else {
